...
public class JavaBank extends JFrame {
    ...
    //a
    //combo box to display the acccount types
    ...
    private void CreateAccountJButtonActionPerformed(ActionEvent event) {
        ...
        try {
            ...
            if(BalanceJTextField.getText()=="0") {
                ...
                assert Integer.parseInt(BalanceJTextField.getText())< 2147483647;
        ...
        if ((noAccounts <= 9) & (Name != "") & (Accountnum != 0))  {
            ...
            //emptyAccount = i;
            displayJTextArea.setText(myAccounts.get(noAccounts).getAccountName() + " " + myAccounts.get(noAccounts).getAccountNum() + " " + myAccounts.get(noAccounts).getBalance());
            noAccounts ++;
            ...
            if (noAccounts == 0) {
                ...
                for (int i=0; i<noAccounts; i++) {
                    if ((myAccounts.get(i).getAccountNum() == Accountnum) && (Deposit>0)) {
                        ...
                        displayJTextArea.setText(myAccounts.get(i).getAccountName() + " " + myAccounts.get(i).getAccountNum() + " " + myAccounts.get(i).getBalance());
                    }
                    ...
                    //если сн€тие меньше 0
                    if(Withdraw<0) {
                        Withdraw=0;
                        throw new MyException("Withdraw less than 0");
                    }
                    else
                        //если депозит меньше 0
                        if(Deposit<0){
                            Deposit=0;
                            throw new MyException("Deposit less than 0");
                        }
                }
            }
...
//    			displayJTextArea.append(myAccounts[i].getaccountname() + " " + myAccounts[i].getaccountnum() + " " + myAccounts[i].getbalance() + "\n");
    ...
    private void displayAccountDetails(AbstractBankAccount abstractBankAccount) {
        displayJTextArea.setText(abstractBankAccount.toString());
